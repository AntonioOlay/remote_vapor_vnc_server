...
vncserver :$VNCDISPLAY -desktop "Desktop $USER" -localhost -AlwaysShared -AcceptKeyEvents \
   -AcceptPointerEvents -AcceptSetDesktopSize -SendCutText -AcceptCutText -SecurityTypes None -xstartup /opt/.vnc/xstartuppanoply
...
test ! -d $HOME/Escritorio/PANOPLY.desktop

if [ $? -gt 0 ]
then
echo "Acceso Directo PANOPLY creado"

else
cp --verbose /opt/.desk_files/PANOPLY.desktop $HOME/Escritorio/
chmod 777 /$HOME/Escritorio/PANOPLY.desktop
chmod u+x /$HOME/Escritorio/PANOPLY.desktop
